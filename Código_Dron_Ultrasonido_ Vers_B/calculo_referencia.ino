...
void Gestion_modos_vuelo(){
    ...
    // Activamos modo acrobatico si estamos en modo ultrasonidos o estable
    else if ((canal_rc[6] >= 1400) && (canal_rc[5] >= 1500)){
      modo_vuelo = FM_Acrobatico;
    }
}
...
void Generacion_referencias(){
  ...
  // Ponemos el modo acrobatico (es posible que se tengan que quitar los PIDs del todo en controladores, ya que es acro no queremos estable):
  else if (modo_vuelo == FM_Acrobatico){
    led_armado(); // Establecemos el led en modo armado
    throttle = canal_rc[3];
    pid_i_mem_altitude = 0;        // Se reinicia los valores del PID para el control de altitud, indicando un vuelo más preciso durante este modo de vuelo.
    pid_last_altitude_d_error = 0;
  }
