...
void setup() {
  ...
  Wire2.setClock(400000);                                      //I2C fast mode (400 kbit/s)
  Wire2.begin();

  Wire2.beginTransmission(MPU6050_address);                    //Comenzar comunicación con sensor
  Wire2.write(0x6B);                                           //Registro a modificar
  Wire2.write(0x00);                                           //Valor asignado a ese registro (00000000)
  Wire2.endTransmission();                                     //Finalizar comunicación con sensor

  //Configurar giroscopio a 500º/s (por defecto 250º/s)
  Wire2.beginTransmission(MPU6050_address);                    //Comenzar comunicación con sensor
  Wire2.write(0x1B);                                           //Registro a modificar
  Wire2.write(0x08);                                           //Valor asignado a ese registro (00001000)
  Wire2.endTransmission();                                     //Finalizar comunicación con sensor

  //Configurar acelerómetro a +/-8g (por defecto +/-2g)
  Wire2.beginTransmission(MPU6050_address);                    //Comenzar comunicación con sensor
  Wire2.write(0x1C);                                           //Registro a modificar
  Wire2.write(0x10);                                           //Valor asignado a ese registro (00010000)
  Wire2.endTransmission();                                     //Finalizar comunicación con sensor
...
void leer_MPU6050() {
  Wire2.beginTransmission(MPU6050_address);                    // Comenzar comunicación con sensor
  Wire2.write(0x3B);                                           // Enviar petición del primer registro
  Wire2.endTransmission();                                     // Finalizar comunicación con sensor
  Wire2.requestFrom(MPU6050_address, 14);                      // Solicitar un total de 14 registros


  while (Wire2.available() < 14);                              // Esperamos hasta recibir los 14 bytes
  acc_x_raw = Wire2.read() << 8 | Wire2.read();            // 0x3B (ACCEL_XOUT_H) & 0x3C (ACCEL_XOUT_L)
  acc_y_raw = Wire2.read() << 8 | Wire2.read();            // 0x3D (ACCEL_YOUT_H) & 0x3E (ACCEL_YOUT_L)
  acc_z_raw = Wire2.read() << 8 | Wire2.read();            // 0x3F (ACCEL_ZOUT_H) & 0x40 (ACCEL_ZOUT_L)
  
  temperature = Wire2.read() << 8 | Wire2.read();          // 0x41 (TEMP_OUT_H)   & 0x42 (TEMP_OUT_L)
  gyro_x_raw = Wire2.read() << 8 | Wire2.read();           // 0x43 (GYRO_XOUT_H)  & 0x44 (GYRO_XOUT_L)
  gyro_y_raw = Wire2.read() << 8 | Wire2.read();           // 0x45 (GYRO_YOUT_H)  & 0x46 (GYRO_YOUT_L)
  gyro_z_raw = Wire2.read() << 8 | Wire2.read();           // 0x47 (GYRO_ZOUT_H)  & 0x48 (GYRO_ZOUT_L)
}
